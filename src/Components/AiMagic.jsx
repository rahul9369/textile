  ...
  return (
    <div className="bg-[linear-gradient(#FBDBB5_97.45%)] px-4 md:px-16 pt-2 pb-16 font-sans text-gray-900">
        ...
        <div className="bg-gradient-to-r relative from-[#fdbb78] to-[#fda769] rounded-xl p-0 md:p-12 flex flex-col lg:flex-row justify-between items-center gap-10">
          ...
          <div className="lg:w-2/3 space-y-6 px-2">
            ...
            <motion.h2
              ...
              <span className="text-white font-bold text-4xl md:text-5xl">AI Magic</span>
            </motion.h2>

            {/* Features */}
            <div className="grid grid-cols-2 gap-3">
              {[
                { img: plus, title: "Generate Magic", desc: "Create textile designs from text" },
                { img: drop, title: "Colorify Instantly", desc: "1-click trendy palette recolor" },
                { img: pen, title: "Edit with Precision", desc: "Scale, crop, remix faster" },
                { img: grid, title: "Grid It. Print It", desc: "Auto-generate repeat patterns" },
              ].map((item, idx) => (
                <div key={idx} className="bg-[linear-gradient(#FBDBB5_97.45%)] flex rounded-md p-3 text-sm shadow-sm">
                  <img src={item.img} alt={item.title} className="w-6 h-6 mx-2 my-auto" />
                  <div>
            ...
            <div className="flex justify-center lg:justify-start pt-3">
              <button className="flex items-center gap-2 px-6 py-3 border text-white rounded-full hover:bg-[#292c33] transition">
                <img src={star} alt="star icon" className="w-5 h-5 filter invert brightness-50" />
                Generate Now
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4">
              {[
                ...
                <div key={idx} className="bg-orange-200 p-4 rounded-2xl shadow-lg text-center">
                  <div className="text-xl md:text-2xl font-bold">{stat.value}</div>
                  <p className="text-xs md:text-xl text-gray-500 mt-1">{stat.label}</p>
                </div>
              ))}
            </div>
      ...
      <div className="text-center mt-16">
        <h3 className="text-xl md:text-4xl font-bold mb-10">Tested & Loved by the Textile Community</h3>
        ...
        <div className="hidden md:grid md:grid-cols-5 gap-6 text-base">
          {[
            ...
            <div key={idx} className="w-40 mx-auto">
              <div className="text-5xl font-bold text-orange-600">{stat.value}</div>
